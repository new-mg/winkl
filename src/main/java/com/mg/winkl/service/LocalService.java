...
import com.mg.winkl.receiver.OnepxReceiver;
...
public final class LocalService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                registerReceiver(screenStateReceiver, intentFilter2, Context.RECEIVER_NOT_EXPORTED);
            }
        } else {
        ...
        if (KeepLive.foregroundNotification != null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                startForeground(13691, notification, ServiceInfo.FOREGROUND_SERVICE_TYPE_SPECIAL_USE);
            }else {
    ...
    private ServiceConnection connection = new ServiceConnection() {
        ...
        public void onServiceDisconnected(ComponentName name) {
            if (ServiceUtils.isServiceRunning(getApplicationContext(), "com.mg.winkl.service.LocalService")) {
